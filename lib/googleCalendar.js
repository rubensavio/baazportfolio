...
/**
 * Calendars to consult for free/busy. The event lives on the organizer
 * (bookings) calendar, but real conflicts also depend on the host's own
 * schedule - so we check both when a separate host is configured.
 */
function getAvailabilityCalendarIds() {
  const { hostEmail } = getGoogleCalendarConfig();
  const insertId = getEffectiveCalendarId();
  const ids = [insertId];
  if (hostEmail && hostEmail !== insertId) ids.push(hostEmail);
  return ids;
}
...
/**
 * Busy blocks for a date across the organizer + host calendars. This is where
 * our own bookings live, so it also reflects slots taken via this widget.
 */
export async function getBusyBlocks(dateStr) {
  ...
  const calendarIds = getAvailabilityCalendarIds();
  ...
  return calendarIds.flatMap(
    (id) => freeBusy.data.calendars?.[id]?.busy || [],
  );
}

/**
 * Drop any externally-sourced slots (e.g. from Calendly) that overlap a Google
 * Calendar busy block - so meetings booked through this widget, which are
 * written to Google, stop being offered even when Calendly still lists them.
 */
export async function filterSlotsByGoogleBusy(dateStr, slots) {
  if (!slots?.length) return slots;
  const busyBlocks = await getBusyBlocks(dateStr);
  if (!busyBlocks.length) return slots;
  return slots.filter((slot) => !overlapsBusy(slot.startIso, slot.endIso, busyBlocks));
}
...
function buildEventBody({ startIso, endIso, name, email, message, usesDelegation, hostEmail }) {
  ...
  const attendees = [{ email: email.trim(), displayName: name.trim() }];
  if (hostEmail && hostEmail.toLowerCase() !== email.trim().toLowerCase()) {
    attendees.push({ email: hostEmail, organizer: false, responseStatus: "accepted" });
  }
  return {
    ...base,
    attendees,
...
export async function createBooking({ date, startIso, name, email, message }) {
  ...
  const { usesDelegation, hostEmail } = getGoogleCalendarConfig();
